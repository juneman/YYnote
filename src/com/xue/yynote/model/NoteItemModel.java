...
public class NoteItemModel {
	...
	public static final String[] mColumns = { ID, CLOCK_ID, CREATE_DATE,
			MODIFY_DATE, CONTENT, AUDIO, SEQUENCE };
	...
	private ClockModel getClockModel(int alertId) {
		...
		Cursor cursor = dbHelper.query(DBHelper.TABLE.ALERTS, null,
				ClockModel.ID + " = " + alertId, null, "");
	...
	public String getContentForItem() {
		...
		while (content.indexOf("[0x64", index) >= 0) {
			...
			content = content.replace(content.substring(index, end + 1),
					"[Pic]");
			index += 1;
		}
		return content;
	...
	public Bitmap getBitmapByTag(String image) {
		// TODO Auto-generated method stub
		File root = this.mContext
				.getExternalFilesDir(Environment.DIRECTORY_PICTURES);
		String filePath = root.getAbsolutePath()
				+ image.substring(5, image.length() - 1);
