...
public class ClockCalendarView extends LinearLayout {
	...
	private void initResource() {
		...
		this.initAdapter();
		this.mAdapter = new CalendarAdapter(this.getContext(),
				R.layout.activity_calendar_item, this.days);
		...
		this.mGridView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				long millis = (Long) ClockCalendarView.this.mAdapter
						.getItem(position);
				ClockCalendarView.this.mDate = millis;
				CalendarItemView curItem = (CalendarItemView) ClockCalendarView.this.mGridView
						.getChildAt(ClockCalendarView.this.curDayPosition
								- ClockCalendarView.this.mGridView
		...
		this.mGridView.setOnScrollListener(new OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				...
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					CalendarItemView item = (CalendarItemView) view
							.getChildAt(28);

					Log.d(TAG,
							"year:" + item.getYear() + " month:"
									+ item.getMonth() + " day:" + item.getDay());
					ClockCalendarView.this.updateYearAndMonth(item.getYear(),
							item.getMonth());
					ClockCalendarView.this.updateCurMonthBg(item.getYear(),
							item.getMonth());
	...
	public void initGridView() {
		...
		this.curItemView = (CalendarItemView) this.mGridView
				.getChildAt(this.curDayPosition);
		...
		this.updateCurMonthBg(this.curItemView.getYear(),
				this.curItemView.getMonth());
	}
	...
	private void updateCurMonthBg(int year, int month) {
		...
		while (item != null
				&& (item.getYear() != year || item.getMonth() != month)) {
			...
			// Log.d(TAG, "" + p + "year:" + item.getYear() + " month:" +
			// item.getMonth() + " day:" + item.getDay());
		}

		item = (CalendarItemView) this.mGridView.getChildAt(p);
		while (item != null && item.getYear() == year
				&& item.getMonth() == month) {
			if ((p + firstVisiblePosition) == this.curDayPosition) {
				item.asCurrentDay();
			} else {
	...
	public void expandData() {
		long lastItem = this.days.get(this.days.size() - 1)
				+ ClockCalendarView.dayOfMillisecond;
		for (int i = 0; i < daysOfMonth; i++) {
			this.days.add(lastItem + ClockCalendarView.dayOfMillisecond * i);
			// this.mAdapter.add(lastItem + ClockCalendarView.dayOfMillisecond *
			// i);
		}
		this.mAdapter.notifyDataSetChanged();
		CalendarItemView item = (CalendarItemView) this.mGridView
				.getChildAt(28);
