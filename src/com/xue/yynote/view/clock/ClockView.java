...
public class ClockView extends LinearLayout {
	...
	private Date time;
	private String[] arrayAlertTimes = this.getContext().getResources()
			.getStringArray(R.array.clock_alert_times_array);
	private String[] arrayAlertInterval = this.getContext().getResources()
			.getStringArray(R.array.clock_alert_interval_array);
				...
				.setItems(arrayAlertTimes,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								ClockView.this.tvAlertTimes
										.setText(arrayAlertTimes[which]);
								ClockView.this.mClockModel
										.setAlertTimes(a[which]);
								if (which > 0
										&& ClockView.this.mClockModel
												.getAlertInterval() == 0) {
									ClockView.this.mClockModel
											.setAlertInterval(1);
									ClockView.this.tvAlertInterval
											.setText(arrayAlertInterval[1]);
								}
							}
						}).show();
	}
				...
				.setItems(arrayAlertInterval,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								// TODO Auto-generated method stub
								int[] a = new int[] { 0, 1, 2, 3, 5, 10 };
								ClockView.this.tvAlertInterval
										.setText(arrayAlertInterval[which]);
								ClockView.this.mClockModel
										.setAlertInterval(a[which]);
							}
						}).show();
	}
	...
	protected void selectMinutes() {
		// TODO Auto-generated method stub
		LinearLayout mTimeView = (LinearLayout) LayoutInflater.from(
				ClockView.this.getContext()).inflate(
				R.layout.activity_clock_time, null);

		final TimePicker localTimePicker = (TimePicker) mTimeView
				.findViewById(R.id.clock_timepicker);
		localTimePicker.setIs24HourView(Boolean.valueOf(true));
		localTimePicker.setCurrentHour(Integer.valueOf(this.time.getHours()));
		localTimePicker
				.setCurrentMinute(Integer.valueOf(this.time.getMinutes()));
				...
				.setPositiveButton(R.string.clock_dialog_ok,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
									int which) {
								// TODO Auto-generated method stub
								ClockView.this.time.setHours(localTimePicker
										.getCurrentHour());
								ClockView.this.time.setMinutes(localTimePicker
										.getCurrentMinute());
								ClockView.this.time.setSeconds(0);
								ClockView.this.tvMinutes.setText(DateUtils.formatDateTime(
										ClockView.this.getContext(),
										ClockView.this.time.getTime(),
										DateUtils.FORMAT_24HOUR
												| DateUtils.FORMAT_SHOW_TIME));
								ClockView.this.mClockModel
										.setTimeInMillis(ClockView.this.time
												.getTime());
							}
	...
	protected void selectDays() {
		...
		final ClockCalendarView mClockCalendar = new ClockCalendarView(
				this.getContext());
				...
				.setPositiveButton(R.string.clock_dialog_ok,
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								ClockView.this.tvDays.setText(DateUtils.formatDateTime(
										ClockView.this.getContext(), millis,
	...
	public void setClockModel(ClockModel clockModel) {
		...
		this.tvMinutes.setText(DateUtils.formatDateTime(
				ClockView.this.getContext(), this.time.getTime(),
		...
		alertTimes += mResources
				.getString(mClockModel.getAlertTimes() > 1 ? R.string.clock_alert_multi_times
						: R.string.clock_alert_single_time);
		this.tvAlertTimes.setText(alertTimes);

		if (mClockModel.getAlertInterval() > 0) {
			String alertInterval = String.valueOf(mClockModel
					.getAlertInterval()) + " ";

			alertInterval += mResources.getString(mClockModel
					.getAlertInterval() > 1 ? R.string.clock_alert_multi_mins
					: R.string.clock_alert_single_min);
			this.tvAlertInterval.setText(alertInterval);
		} else
			this.tvAlertInterval.setText(R.string.clock_none);
	}
