...
public class NoteEditView extends LinearLayout implements OnClickListener{
			...
			public void onClick(View v) {
				...
				this.mPlayer.setOnCompletionListener(new OnCompletionListener(){
					...
					public void onCompletion(MediaPlayer mp) {
						// TODO Auto-generated method stub
						NoteEditView.this.mAudioPlay.setBackgroundResource(R.drawable.audiologo);
						mPlayer.stop();
						mPlayer.release();
						mPlayer = null;
						anim.stop();
		        ...
		        try {
		            mPlayer.setDataSource(NoteEditView.this.mNoteItemModel.getAudio());
		            mPlayer.prepare();
		            mPlayer.start();
		            NoteEditView.this.mAudioPlay.setBackgroundResource(R.anim.audio_play);
		            anim = (AnimationDrawable) NoteEditView.this.mAudioPlay.getBackground();
		            anim.stop();
		            anim.start();
		        } catch (IOException e) {
		            e.printStackTrace();
	...
	protected void addAudio() {
		...
		hideSoftInputFromWindow(this.getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
		final NoteEditAudioView audioView = new NoteEditAudioView(this.getContext());
		Dialog dlg = new AlertDialog.Builder(this.getContext()).
			...
			setPositiveButton(R.string.ok_btn, new DialogInterface.OnClickListener(){
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					NoteEditView.this.mNoteItemModel.setAudio(mRecordFile);
					
					ContentValues cv = new ContentValues();
					cv.put(NoteItemModel.AUDIO, mRecordFile);
					DBHelper dbHelper = DBHelper.getInstance(NoteEditView.this.getContext());
					dbHelper.update(DBHelper.TABLE.NOTES, cv, NoteItemModel.ID + " = " + NoteEditView.this.mNoteItemModel.getId(), null);
					
					NoteEditView.this.mAudioPlay.setVisibility(VISIBLE);
				}
	...
	public void addClockFunction(){
		...
		if(this.mNoteItemModel.getClockId() < 0 && this.mNoteItemModel.getId() >= 0){
			...
			dbHelper.update(DBHelper.TABLE.NOTES, this.mNoteItemModel.formatContentValuesWithoutId(), 
					NoteItemModel.ID + " = " + NoteEditView.this.mNoteItemModel.getId(), 
					null);
		}
	...
	private void setContent(String content) {
		...
		for(String image : images){
			Bitmap bitmap = this.mNoteItemModel.getBitmapByTag(image);
